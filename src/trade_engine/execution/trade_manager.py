...
class TradeManager:
    ...
    def flatten_all(self, positions: Iterable[Position]) -> None:
        for position in positions:
            action = "SELL" if position.side.value == "long" else "BUY"
            order_id = self.executor.submit(
                symbol=position.symbol,
                action=action,
                quantity=round(abs(position.quantity), 2),
            )
            self.database.log(
                category="execution",
                message=f"Flattened {position.symbol}",
                payload=json.dumps({"order_id": order_id, "quantity": position.quantity}),
            )
